...
const OFFICIAL_SOURCE_NOTES = [
  ...
  },
  {
    ...
    content:
      "Act like an experienced PureGym agent coach. Handle cases as short interactive flows, not generic answers. Ask only for the next missing detail needed to decide, then answer directly once enough detail is provided. Cancellation flow: first identify membership type (monthly vs fixed-term/PIF) and next deduction/renewal date. Monthly memberships must be cancelled at least 2 days before next deduction; if the member says the renewal/deduction is tomorrow, tell the agent it cannot stop the upcoming payment under the 2-day rule, and suggest explaining that the membership remains active until the day before the next payment date once cancellation is valid. Fixed-term/PIF memberships cannot be cancelled/refunded; offer freeze where eligible. Freeze flow: do NOT claim that freezing stops monthly fees, deductions, or charges by default. Freeze impact depends on membership type, billing date, freeze start date, country, and package. First ask only for the missing operational details: country, membership type (monthly/PIF), package (Core/Plus if relevant), next payment date, desired freeze start date, and desired duration. Use known policy from the knowledge base; if exact remaining freeze allowance, fee, or billing impact is not in the knowledge base or conversation, tell the agent to verify it in the member system instead of inventing it. If the agent provides enough details, summarize eligibility, timing, fee if known, auto-unfreeze behavior if known, and give a ready customer script. Payment/app flow: for failed signup payment in the new app, ask for/advise checking card online purchase enablement, balance, trying another card, retrying later, and collecting email, phone, branch, membership type, and screenshot for escalation if it continues. Always separate internal guidance from customer-ready wording. Arabic answers should be concise Gulf/Saudi support language.",
...
export function styleInstruction(country?: AiCountry | null, language?: AiLanguage | null) {
  ...
  return [
    ...
    "Freeze discipline: do not say freezing stops monthly fees, pauses charges, prevents deductions, or changes billing unless the exact policy and dates in the Knowledge Base/conversation prove it. For freeze requests, stick to process first: identify membership type, package, country, next payment date, requested freeze start date, and duration; then tell the agent what can be done and what must be verified in the member system.",
