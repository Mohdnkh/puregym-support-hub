  ...
  return (
    <div className="admin-layout-v2">
      <div className="card admin-panel">
        <div className="section-head"><div><h2>Script order</h2><p>Drag scripts inside one category, then save order.</p></div><button className="btn small" onClick={saveOrder}>Save order</button></div>
        <select className="select" value={adminCategory} onChange={(e) => setAdminCategory(e.target.value)}>{adminCategories.map((cat) => <option key={cat}>{cat}</option>)}</select>
        <div className="admin-list draggable-list">
          {adminScripts.map((script) => <button key={script.id} draggable onDragStart={() => setDraggedId(script.id)} onDragOver={(e) => e.preventDefault()} onDrop={() => reorderLocal(script.id)} className={`script-card ${selected?.id === script.id ? "active" : ""}`} onClick={() => setSelectedId(script.id)}><b>☰ {script.title}</b><p>{script.country} • {script.language} • #{script.sortOrder || 0}</p></button>)}
        </div>
      </div>

      <div className="card admin-editor">
        <div className="section-head"><div><h2>Edit script</h2><p>Changes apply to all users.</p></div><div className="toolbar"><button className="btn ghost small" onClick={() => selected && deleteScript(selected.id)} disabled={!selected}>Delete</button><button className="btn small" onClick={saveSelected} disabled={!selected}>Save</button></div></div>
        {status && <p className={status.includes("failed") || status.includes("error") ? "error" : "success"}>{status}</p>}
      ...
      <div className="card admin-panel users-panel">
        ...
        <div className="admin-list">
          {users.map((member) => <div className="user-row" key={member.id}><div><b>{member.nameEn} / {member.nameAr}</b><span>{member.email} • {member.role} • {member.emailVerifiedAt ? "Verified" : "Not verified"}</span></div><button className="btn ghost small" onClick={() => deleteUser(member.id)} disabled={member.id === currentUser.id}>Delete</button></div>)}
        </div>
      </div>
    </div>
  );
